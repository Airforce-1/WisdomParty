...
import wuxc.wisdomparty.Adapter.PartyRespondAdapter.Callback;
...
public class MemberRewardsActivity extends Activity implements OnTouchListener,Callback, OnClickListener, OnItemClickListener {
	...
	private void GetData() {
		...
		chn = GetChannelByKey.GetSign(PreALLChannel, "党员奖惩");
	...
	protected void go() {
		ListData.setPadding(0, -100, 0, 0);
		mAdapter = new PartyRespondAdapter(this, list, ListData,this);
	...
	public void click(View v) {
		...
		switch (v.getId()) {
		case R.id.lin_all:
			RespondModel data = list.get((Integer) v.getTag());
			if (data.isCont()) {
				Intent intent = new Intent();
				intent.setClass(getApplicationContext(), AssistanceDetailActivity.class);
				Bundle bundle = new Bundle();
